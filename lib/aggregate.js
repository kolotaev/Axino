...
const {
  SnapshotTakingIsDone, NonUniqueAggregateId,
  AggregatesNotFound, AggregateHasUncommittedEvents,
} = require('./errors');
...
class AggregateRepository {
  ...
  loadAggregate(id) {
    return _.first(this.loadAggregates(id));
  }

  contains(aggregateID) {
    return Config.get.eventStore.streamExists(aggregateID);
  }

  commit(cmd) {
    const updatedAggregates = _.remove(this._store.all(), a => a.uncommittedEvents.length === 0);
    if (updatedAggregates.length === 0) {
      return;
    }
    const streamsWithEvents = _.chain(updatedAggregates)
      .map(a => [a.eventStream, a.uncommittedEvents])
      .tap(a => a.clearUncommittedEvents());

    Config.get.eventStore.commitEvents(cmd, streamsWithEvents);
  }

  clear() {
    // todo - better handle UoW
    if (_.some(this._store.all(), a => a.uncommittedEvents > 0)) {
      throw new AggregateHasUncommittedEvents();
    }
    this._store.clear();
  }
}
